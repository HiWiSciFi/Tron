...
public class GameManager : MonoBehaviour
{
    ...
    public void Switcher(byte[] buffer)
    {
        if (buffer[0] == 0)
        {
            // standard
            ...
            for (int i = 0; i < pcs.Count; i++)
            {
                if (pcs[i].ID == ID)
                {
                    ...
                    pcs[i].transform.rotation = new Quaternion(pcs[i].transform.rotation.x, yAngle, pcs[i].transform.rotation.z, pcs[i].transform.rotation.w);
